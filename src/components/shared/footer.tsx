...
export function Footer() {
  ...
  if (recipe.footer === 'editorial-footer') {
    return (
      <footer className="border-t border-neutral-200/70 bg-[#f2f8f5] text-neutral-950">
        <div className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
          <div className="grid gap-10 lg:grid-cols-[1.1fr_0.9fr_0.9fr]">
            <div>
              <div className="inline-flex items-center gap-2 rounded-full border border-neutral-200/90 bg-white/70 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.22em] text-neutral-600">
                <Sparkles className="h-3.5 w-3.5 text-neutral-500" />
                Editorial desk
              </div>
              <h3 className="mt-5 text-3xl font-semibold tracking-tight">{SITE_CONFIG.name}</h3>
              <p className="mt-4 max-w-md text-sm leading-7 text-neutral-600">{SITE_CONFIG.description}</p>
            </div>
            <div>
              ...
              <ul className="mt-4 space-y-3 text-sm text-neutral-800">
                {footerLinks.platform.map((item: any) => (
                  <li key={item.name}>
                    <Link href={item.href} className="transition-colors hover:text-neutral-950">
                      {item.name}
                    </Link>
                  </li>
                ))}
          ...
          <div>
            <Link href="/" className="flex items-center gap-3">
              <div className="h-11 w-11 overflow-hidden rounded-2xl border border-slate-200 bg-white p-1 shadow-sm">
                <img src="/favicon.png?v=20260420" alt={`${SITE_CONFIG.name} logo`} width="44" height="44" className="h-full w-full object-contain" />
              </div>
              <div>
